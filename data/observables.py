...
class Observables(object):
	def __init__(self, caption = '', use_acceptance = False):
		...
		self.rapidity = ROOT.TH1F('hEta%s' % unique_identifier, '#eta', 1000, -2, 2)
		self.phi      = ROOT.TH1F('hPhi%s' % unique_identifier, 'Angle #phi between d and (#pi^{+}, #pi^{-})'+ caption + '; #phi ' , 1000, 0,  2 * ROOT.TMath.Pi() )
		self.mass     = ROOT.TH1F('hMass%s' % unique_identifier, 'Effective mass ' + caption, 1000, 0.1, 2.7) 
		self.momtot   = ROOT.TH1F('hMomt%s' % unique_identifier, 'Momentum of the system (d, #pi^{-}, #pi^{+})' + caption, 5000, 0, 10) 
		self.sdalitz  = ROOT.TH2F('Msdalitz%s' % unique_identifier, 'x vs y %s; #sqrt{3} (T_{#pi^{+}} - T_{#pi^{-}})/q; (2T_{d} - T_{#pi^{+}} - T_{#pi^{-} })/q ' % caption, 2000, -6, 6, 2000, 0, 12.);
		self.dalitz   = ROOT.TH2F('M_dalitz%s' % unique_identifier, 'M(d, #pi^{+})^{2} vs M(#pi^{+}, #pi^{-})^{2}' + caption, 2000, 0.05, 0.25, 2000, 4., 5.);
		self.my       = ROOT.TH2F('M_vs    %s' % unique_identifier, 'M(d, #pi^{-}, #pi^{+})^{2} vs M(#pi^{-}, #pi^{+})^{2}' + caption, 2000, 0.05, 0.45, 2000, 0.3, 7.,);

		# Hardcoded for now
		mass_of_interest = 2.370
		fT = lambda x : x.E() - x.M()     # kinetic energy
		fq = lambda x, y, z: mass_of_interest - x.M() - y.M() - z.M() # defect mass
	...
	def InDetector(self, particle, deuteron = False):
		if not self.use_acceptance: return True

		# acceptance cuts
		if abs( particle.Eta() ) > 0.8: return False
		if particle.Pt() < 0.15:  return False
		if deuteron and particle.P() > 1.3: return False
		if particle.Pt() > 2.: return False
	...
	def PushBack(self, deuterons, positive, negative):
		...
		if deuterons and positive and negative: self.FillHistograms(deuterons, positive, negative)
